## API Report File for "@ridha.bouazizi/oidc-client-ts-redis"
...
export class RedisStateStore implements StateStore {
    // Warning: (ae-forgotten-export) The symbol "RedisClientLike" needs to be exported by the entry point index.d.ts
    constructor(redisClient: RedisClientLike, config?: RedisConfig);
    // (undocumented)
...
export class ServerSideOidcClient {
    constructor(config: ServerSideOidcConfig);
    buildUser(signinResponse: SigninResponse): User;
    cleanupExpiredState(): Promise<void>;
    get client(): OidcClient;
    createSigninRequest(args?: Parameters<OidcClient['createSigninRequest']>[0]): Promise<SigninRequest>;
    processSigninResponse(callbackUrl: string): Promise<SigninResponse>;
    get stateStore(): RedisStateStore;
}
...
export class ServerSideUserManager {
    constructor(config: ServerSideOidcConfig);
    createSigninRequest(args?: Parameters<OidcClient['createSigninRequest']>[0]): Promise<SigninRequest>;
    getUserSession(sessionKey: string): Promise<User | null>;
    hasUserSession(sessionKey: string): Promise<boolean>;
    removeUserSession(sessionKey: string): Promise<void>;
    get serverClient(): ServerSideOidcClient;
    signinCallback(callbackUrl: string): Promise<User>;
    storeUserSession(sessionKey: string, user: User, ttl?: number): Promise<void>;
}
